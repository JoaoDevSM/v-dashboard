...
import { routes } from '../composables/routes.js'
        ...
        <div class="flex items-center">
          <svg class="w-12 h-12" viewBox="0 0 512 512" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              ...
              fill="#0a90bd" stroke="#0a90bd" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
            <path
              d="M201.694 387.105C231.686 417.098 280.312 417.098 310.305 387.105C325.301 372.109 332.8 352.456 332.8 332.8C332.8 313.144 325.301 293.491 310.305 278.495C295.309 263.498 288 256 275.2 230.4C256 243.2 243.201 320 243.201 345.6C201.694 345.6 179.2 332.8 179.2 332.8C179.2 352.456 186.698 372.109 201.694 387.105Z"
              fill="white" />
          </svg>

          <span class="mx-2 text-2xl font-semibold text-white">AquaSignal</span>
        </div>
      </div>

      <nav class="mt-10 space-y-1">
        <button :class="{ 'bg-primary': route.route.split('/')[1] === router.path.split('/')[1] }"
          @click="$router.push(route.route)" v-for="route in routes"
          class="w-full flex items-center duration-200 hover:bg-primary/40 bg-gray-700/20 p-1 py-2">
          <span class="mx-4 text-white text-sm">{{ route.name }}</span>
        </button>
